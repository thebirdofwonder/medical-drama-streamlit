...
MAX_VOICEVOX_CHARS = 200  # 長文を分割して送る目安
...
def run_script_review(script: str) -> dict[str, Any]:
    api_key = get_api_key()
    if api_key:
        return review_with_claude(script, api_key)
...
def concat_wav_bytes(wav_list: list[bytes], pause_ms: int = 350) -> bytes:
    """複数WAVを無音つきでつなげる。"""
    if not wav_list:
        raise ValueError("音声データが空です。")
    params = None
    frames = []
    for i, data in enumerate(wav_list):
        with wave.open(io.BytesIO(data), "rb") as w:
            ...
            if i < len(wav_list) - 1 and pause_ms > 0:
...
def generate_narration_wav(script: str) -> bytes:
    ...
    for i, chunk in enumerate(chunks):
        st.session_state["_voice_progress"] = f"{i + 1}/{len(chunks)}"
        wavs.append(synthesize_wav_bytes(chunk))
    return concat_wav_bytes(wavs)
...
def create_background_image(
    ...
    """
    暗い医療ドラマ背景に、サムネ風のタイトル／サブタイトルを描画。
    右下の VOICEVOX クレジットは必ず残す。
    """
    w, h = VIDEO_SIZE
    ...
    for i in range(8):
        yy = int(h * 0.25) + i * 70
        draw.line([(80, yy), (w - 80, yy)], fill=(40, 70, 90), width=1)

    # --- YouTubeサムネ風: タイトル（上・ピンク）／サブタイトル（下寄り・黄or水色）---
    ...
    x = w - tw - margin
    y = h - th - margin
    draw.text((x + 2, y + 2), CREDIT_TEXT, font=credit_font, fill=(0, 0, 0))
    draw.text((x, y), CREDIT_TEXT, font=credit_font, fill=(210, 220, 230))
...
def build_mp4(
    ...
    # Apple Silicon: まず VideoToolbox、だめなら libx264
    ...
    encode_attempts = [
        {
            ...
            "audio_codec": "aac",
            "ffmpeg_params": ["-b:v", "6M", "-pix_fmt", "yuv420p"],
        },
        {
            "codec": "libx264",
            "audio_codec": "aac",
...
def main() -> None:
    ...
    st.write(
        ...
        "AIレビュー → VOICEVOX音声 → 背景・BGM合成 → MP4ダウンロード、まで進めます。"
    )
    ...
    if st.session_state.script_confirmed:
        st.header("ステップ3: 音声・画像・BGMを合成してMP4を作る")
        st.caption(
            "VOICEVOX（話者ID 84 / 青山龍星）→ タイトル付き背景＋クレジット → BGM → MP4"
        )

        st.subheader("画面に出すタイトル（YouTubeサムネ風）")
        st.text_input(
            "タイトル（大きめ・ピンク）",
            key="video_title",
            help="動画の背景上に大きく表示されます",
        )
        st.text_input(
            "サブタイトル（黄／水色）",
            key="video_subtitle",
            help="タイトルの下寄りに表示されます",
        )
        st.caption(f"右下のクレジットは残します: {CREDIT_TEXT}")
        ...
        if st.button("3. 動画を生成する", type="primary"):
            ...
            try:
                ...
                with tempfile.TemporaryDirectory(prefix="meddrama_") as tmp:
                    ...
                    status.info("① 音声を生成しています（VOICEVOX）…")
                    progress.progress(15, text="VOICEVOX で音声生成中…")
                    wav_bytes = generate_narration_wav(st.session_state.final_script)
                    wav_path = tmp_path / "narration.wav"
                    wav_path.write_bytes(wav_bytes)

                    status.info("② 背景にタイトル・サブタイトル・クレジットを描いています…")
                    progress.progress(40, text="背景画像を生成中…")
                    ...
                    progress.progress(55, text="BGMを取得中…")
                    bgm_path = ensure_bgm(WORK_DIR)

                    status.info("④ 動画に結合しています（少し時間がかかります）…")
                    progress.progress(70, text="MP4へエンコード中…")
                    out_path = tmp_path / "medical_drama.mp4"
                    ...
                    status.success("動画の生成が完了しました。下のボタンからダウンロードできます。")
